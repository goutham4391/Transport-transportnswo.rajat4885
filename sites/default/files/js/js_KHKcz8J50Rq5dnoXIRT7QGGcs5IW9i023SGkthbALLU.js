...
;(function( $ ) {var $listing = $('[data-event-tile-listing]');
	var $breakpoint = $('html').data('current-breakpoint');
	...
	function init() {
		...
		$(window).on('tfnswBreakpointChange', function(e){
			$breakpoint = (e.detail && e.detail.breakpoint) || e.originalEvent.detail.breakpoint;
		});

		if ( $listing.length > 0 ){
			$listing.each(function(){
				var $this = $(this);
				var source = $this.attr('data-tile-listing-source');

				if ( source ){
					var moreLabel = $this.attr('data-tile-listing-more-label');
					var mobPP = $this.attr('data-tile-listing-mobile-per-page');
					var deskPP = $this.attr('data-tile-listing-desktop-per-page');
					page = 1;

					$this.append( loadMoreButton(moreLabel) );

					$this.find('[data-tile-listing-more-button] button').on('click', _.debounce(function(){
						if ( $breakpoint === 'xs' ){
							loadItems(source, mobPP, $this);
						} else {
							loadItems(source, deskPP, $this);
						}
					}, 500));
				}
			});
		...
		function loadItems(source, perPage, $this) {
			var $list = $this.find('[data-tile-listing-list]');
			console.log(page);

			$this.find('[data-tile-listing-more-button]').before('<div class="text-center h3" data-tile-listing-loading>Loading…</div>');
			...
			$.ajax({
				...
				$this.find('[data-tile-listing-loading]').remove();

				if ( !response.length ){
					$this.find('[data-tile-listing-more-button]').remove();
				} else {
					$.each(response, function(index, value){
						if ( index < perPage ){
							$list.append( listingItem(value) );
						}
					});
				}
				...
				console.log('There was an error retrieving more items.');
			});
		}

		function loadMoreButton(label) {
			label = label !== undefined ? label : 'Load more';

			var html = '<div class="text-center" data-tile-listing-more-button>'
				+ '<button type="button" class="btn-more">' + label + '</button>'
				+ '</div>';
		...
		function listingItem(item) {
			var html = '<div class="col-xs-12 col-sm-6 col-lg-4 tile-item-wrapper">';
			html += '<div class="tile-item';
			if ( item.isdatetimeoverlay ) html += ' tile-item--datetime-overlay';
			if ( item.isnews ) html += ' tile-item--news';
			if ( item.isevent ) html += ' tile-item--event';
			html += '">';

			if ( item.imagemarkup ){
				console.log(item.imagemarkup)
				html += '<a href="' + item.target + '" class="tile-item--media">'
					+ item.imagemarkup
					+ '</a>';
			}

			if ( item.title ){
				html += '<div class="tile-item--heading">';

				if ( item.title ){
					html += '<h2 class="tile-item--title">';
					if ( !item.linked ) html += '<a href="' + item.target + '">';
					html += item.title;
					if ( !item.linked ) html += '</a>';
					html += '</h2>';
				}

				if ( item.dateone ){
					html += '<h3 class="tile-item--subtitle">' + item.dateone;
					if ( item.datetwo ) html += item.datetwo;
					html += '</h3>';
				}

				if ( item.isdatetimeoverlay ){
					html += '<div class="tile-item--datetime h3">'
						+ '<span class="overlay-hide">' + item.dateone + ' ' + item.datetwo + '</span>'
						+ '<span class="overlay-show">' + item.dateshort + '</span>'
						+ '</div>';
				}
			...
			html += '<div class="tile-item--body">';
			if ( item.body ){
				html += '<p>' + item.body + '</p>';
				html += '<p><a href="' + item.target + '" class="btn btn-link">Read more</a></p>';
			}
			html += '</div>';
...
/**
 * @file
 * Javascript which manages the logic
 * for event category and region filters
 * of events and news pages
 */
...
(function ($, Drupal) {
  ...
  $(function () {
    ...
    var drupalViewName = drupalSettings.tfnsw_views_filter.components.view;
    var buttonId;
    var inputPrefixNameRegion;

    //Override selectors for operators.
    if(drupalViewName == 'transport_operators_view') {

      buttonId = '#transport-operator-filter-submit';
      inputPrefixNameRegion = 'field_operator_regions_target_id';

    } else {

      buttonId = '#views-exposed-form-' + drupalViewName.replace(/_/g, '-') + '-index button[type=submit]';
      inputPrefixNameRegion = 'field_regions_target_id';
    }

    //Event selectors
    var lSInputKeyEvent = drupalViewName + '-' + 'event-filter-inputs-checked';
    var inputPrefixNameEvent = 'field_categories_target_id';
    var checkBoxesWrapperIdEvent = '#event-filter';
    var buttonWrapperCollapseEvent = '#event-filter-result';
    //Region selectors
    var lSInputKeyRegion = drupalViewName + '-' + 'region-filter-inputs-checked';
    var checkBoxesWrapperIdRegion = '#region-filter';
    var buttonWrapperCollapseRegion = '#region-filter-result';
    //Operator selectors
    var lSInputKeyOperator = drupalViewName + '-' + 'operator-filter-inputs-checked';
    var inputPrefixNameOperator = 'field_type_target_id_1';
    var checkBoxesWrapperIdOperator = '#transport-operator-filter';
    var buttonWrapperCollapseOperator = '#transport-operator-filter-result';

    //Link with URL parameters overrides local storage values.
    if(location.href.indexOf('?') >= 0){

      localStorage.setItem(lSInputKeyEvent, null);
      localStorage.setItem(lSInputKeyRegion, null);
      localStorage.setItem(lSInputKeyOperator, null);
      var newURL = location.href.split("?")[0];
      window.history.pushState('object', document.title, newURL);
    }

    //Loads the saved filters from local storage.
    pickupPreviousFilters(lSInputKeyEvent, checkBoxesWrapperIdEvent, inputPrefixNameEvent, buttonId);
    pickupPreviousFilters(lSInputKeyRegion, checkBoxesWrapperIdRegion, inputPrefixNameRegion, buttonId);
    pickupPreviousFilters(lSInputKeyOperator, checkBoxesWrapperIdOperator, inputPrefixNameOperator, buttonId);
    //Trigger search after loading values from local storage.
    $(buttonId).click(function(){
        $(document).ajaxSuccess(function (event, request, settings) {

          if (settings.extraData != null && settings.extraData.view_name != null && settings.extraData.view_name == drupalViewName) {

            //Saves the filters in local storage.
            saveSelectedFilters(lSInputKeyEvent, checkBoxesWrapperIdEvent, inputPrefixNameEvent, buttonWrapperCollapseEvent);
            saveSelectedFilters(lSInputKeyRegion, checkBoxesWrapperIdRegion, inputPrefixNameRegion, buttonWrapperCollapseRegion);
            saveSelectedFilters(lSInputKeyOperator, checkBoxesWrapperIdOperator, inputPrefixNameOperator, buttonWrapperCollapseOperator);
            var filterHasResult = $(".view-content ul.row");

            //Shows no result message.
            if(filterHasResult.length == 0){

              var emptyMessage = '';

              if(drupalViewName.includes('events')) {

                emptyMessage = 'There are currently no events that match your selected filters.';
              }

              if(drupalViewName.includes('news')) {

                emptyMessage = 'There are currently no news articles that match your selected filters.';
              }

              $(".view-filters .form-group").after("<div class=\"\">"+emptyMessage+"</div>");
            }
          }
        });

        if (typeof Drupal.Ajax != "undefined" &&
          typeof Drupal.Ajax.prototype != "undefined") {

          //Disables filter inputs while loading the search results and shows a progress text searching.
          Drupal.Ajax.prototype.beforeSubmit = function (form_values, element, options) {

            $(".view-id-"+drupalViewName+".view-display-id-index .view-content").html(
              '<div class="list-group">' +
              '<div class="list-group-item text-center" data-search-suggestions-searching="">Searching…</div>' +
              '</div>'
            );
            element.find('input[type="checkbox"]').closest('.checkbox-inline-custom').addClass('disabled').attr('disabled');
            element.find('input[type="checkbox"]').addClass('disabled').attr('disabled', 'true');
          ...
          // Add loading ajax-progress text to load move events.
          Drupal.Ajax.prototype.setProgressIndicatorFullscreen = function () {
            if (typeof this.element !== 'undefined' && this.element.innerText === 'Load More') {
              this.progress.element = $('<div class="ajax-progress text-center h3">Loading…</div>');
              $('.js-pager__items .pager__item').before(this.progress.element);
            }
          };
  ...
  /**
   * Picks up values from local storage to set the filter to remembered state.
   *
   * @param lSInputKey
   * @param checkBoxesWrapperId
   * @param inputPrefixName
   * @param buttonId
   */
  function pickupPreviousFilters(lSInputKey, checkBoxesWrapperId, inputPrefixName, buttonId){

    var oldKey = localStorage.getItem(lSInputKey);

    if(oldKey != null){

      $(checkBoxesWrapperId).find('input[name^='+inputPrefixName+']').each(function(index, item){
  ...
  /**
   * Saves selected checkboxes in local storage.
   *
   * @param lSInputKey
   * @param checkBoxesWrapperId
   * @param inputPrefixName
   * @param buttonWrapperCollapse
   */
  function saveSelectedFilters(lSInputKey, checkBoxesWrapperId, inputPrefixName, buttonWrapperCollapse){

    var appTagHistoryCount = 0;
    localStorage.setItem(lSInputKey, '');

    $(checkBoxesWrapperId).find('input[name^='+inputPrefixName+'][checked="checked"]').each(function(index, item){

      var oldKey = localStorage.getItem(lSInputKey);
      var newKey = (oldKey != null && oldKey.length) > 0 ? oldKey + ',' + $(item).attr('name') : $(item).attr('name');
      localStorage.setItem(lSInputKey, newKey);
      appTagHistoryCount++;
    });

    if(appTagHistoryCount > 0) {

      expandFilter(checkBoxesWrapperId, buttonWrapperCollapse, true);
    } else {

      expandFilter(checkBoxesWrapperId, buttonWrapperCollapse, false);
    }
  }

  /**
   * Expand or collapse the collapsible container around filter inputs.
   *
   * @param checkBoxesWrapperId
   * @param buttonWrapperCollapse
   * @param expand
   */
  function expandFilter(checkBoxesWrapperId, buttonWrapperCollapse, expand){
      if(expand) {
          $(checkBoxesWrapperId).addClass('in');
          $(buttonWrapperCollapse).removeClass('collapsed');
          $(buttonWrapperCollapse).attr("data-change-me","someValue");
          $(buttonWrapperCollapse).attr("aria-expanded","true");
      } else {
          $(checkBoxesWrapperId).removeClass('in');
          $(buttonWrapperCollapse).addClass('collapsed');
          $(buttonWrapperCollapse).attr("aria-expanded","false");
      }
  }
})(jQuery, Drupal);
;
