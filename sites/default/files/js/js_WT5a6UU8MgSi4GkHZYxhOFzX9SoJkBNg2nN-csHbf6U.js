...
/**
 * @file
 * Javascript behaviors for the TFNSW Base module.
 */
...
(function ($, Drupal, window) {
  ...
  Drupal.behaviors.iFrameResize = {
    attach: function (context) {
      $('iframe.embedded-iframe').iFrameResize();
...
function receiveMessage(event) {
  if (event.data == 'scrollToAckMsg' || event.data == 'scrollToErrMsg') {
    scrollTo(0, 0);
  }
  if(event.data == 'scrollToAckMsg') {
    document.title = document.title.replace('feedback', 'feedback submitted');
  }else if(event.data == 'scrollToErrMsg') {
    document.title = document.title.replace('feedback', 'feedback failed');
  }
}
;
/**
