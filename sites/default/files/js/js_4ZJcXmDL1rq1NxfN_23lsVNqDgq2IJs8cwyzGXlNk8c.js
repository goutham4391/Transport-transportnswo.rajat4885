...
/**
 * @file
 * Javascript behavior for the fare block.
 */
(function ($, Drupal, window) {
  'use strict';

  Drupal.behaviors.fareBlock = {

    fare_block: null,

    attach: function (context) {
      var self = this;
      self.fare_block = $(context).find('.regional-specials');

      self.fare_block.once('regional-specials').each(function () {
        var $filter = self.fare_block.find('.regional-specials--selector');
        // If filter found run init();
        if ($filter.length > 0) {
         self.init();
        }
      });

      self.fare_block.on('click', '.tile-item--regional-special a', function(e) {
        e.preventDefault();
        self.changeFareSessionStorage($(this));
      });
    },

    init: function() {
      var self = this;
      var $filter = self.fare_block.find('.regional-specials--selector .dropdown-menu');

      // Create onclick event on filter links.
      $filter.on('click', 'a', function(e) {
        e.preventDefault();
        var location = $(this).attr('data-destination');

        self.displayCards(location);
        self.changeFilterLabel($(this).text());
      });
    },
    /**
     * Display cards based of filter location.
     *
     * @param location
     */
    displayCards: function(location) {
      var self = this;
      var $cards = self.fare_block.find('[data-tile-listing-list]');

      // If 'All destination' was selected then show all.
      if (location === '_ALL') {
        $cards.find('.tile-item-wrapper').show();
        //Hide destination cards on all option
        $cards.find('.destination-cards').hide();
        return;
      }
      else {
        // If a destination was selected.
        // 1. Hide cards.
        $cards.find('.tile-item-wrapper').hide();
        // 2. Only display a selected destination.
        $cards.find('[data-destinations*=' + location + ']').show();
        return;
      }
    },

    /**
     * Change label in "Fare Specials for: *".
     *
     * @param label
     */
    changeFilterLabel: function(label) {
      var self = this;
      var $filter = self.fare_block.find('.regional-specials--selector');
      $filter.find('.filter-label').text(label);
    },

    /**
     * Update sessionStorage with new fare block info.
     *
     * @param target
     */
    changeFareSessionStorage: function (target) {
      if (target.is('a')) {
        var from = target.parent().attr('data-from');
        var fromLabel = target.parent().attr('data-from-label');
        var to = target.parent().attr('data-to');
        var toLabel = target.parent().attr('data-to-label');
        if (typeof from !== typeof undefined && typeof to !== typeof undefined &&
           typeof fromLabel !== typeof undefined && typeof toLabel !== typeof undefined) {
          var fareJson = {
          ...
          sessionStorage.setItem('regional-booking', JSON.stringify(fareJson));
        }
